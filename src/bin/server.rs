...
use std::io::{BufRead, BufReader, Write, SeekFrom};
...
struct Args {
    ...
    /// Number of batches between database snapshots (0 to disable)
    #[arg(long, default_value_t = 0)]
    snapshot_interval: u64,
}

fn handle_client(args: Arc<Args>, stream: TcpStream, map: Arc<RwLock<TreeMap<String, String>>>, log_file: Option<Arc<Mutex<File>>>, batch_counter: Arc<Mutex<u64>>) {
    ...
    while let Some(Ok(line)) = lines.next() {
        ...
        match parts[0] {
            ...
            "SET" if parts.len() == 3 => {
                ...
                log_buffer.push_str(&format!("SET {} {}\n", parts[1], parts[2]));
                response.push_str("OK\r\n");
            }
            "REMOVE" if parts.len() == 2 => {
                let mut map = map.write().unwrap();
                match map.remove(&parts[1].to_string()) {
                    Some(_) => {
                        log_buffer.push_str(&format!("REMOVE {}\n", parts[1]));
                        response.push_str("OK\r\n");
                    }
            ...
            "ENDBATCH" => {
                if let Some(log_mux) = &log_file {
                    if !log_buffer.is_empty() {
                        let mut log = log_mux.lock().unwrap();
                        log.write_all(log_buffer.as_bytes()).unwrap();
                    }
                }
                writer.write_all(response.as_bytes()).unwrap();
                response=String::new();
                {
                    let mut count = batch_counter.lock().unwrap();
                    *count += 1;
                    if args.snapshot_interval > 0 && *count % args.snapshot_interval == 0 {
                        let map_locked = map.read().unwrap();
                        if let Err(e) = map_locked.save_to_file(&args.dbfile) {
...
fn main() -> std::io::Result<()> {
    ...
    let log_file = if args.memonly {
        None
    } else {
        Some(Arc::new(Mutex::new(OpenOptions::new().append(true).create(true).open(&args.logfile)?)))
    };
        ...
        match stream {
            Ok(s) => {
                ...
                if args.singlethread {
                    handle_client(args, s, map, log_file, batch_counter);
                }
                else { 
                    std::thread::spawn(move || { handle_client(args, s, map, log_file, batch_counter); });
                }
            }
